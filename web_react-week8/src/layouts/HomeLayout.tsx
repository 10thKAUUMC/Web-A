...
export default function HomeLayout() {
  ...
  const { isOpen: isSidebarOpen, open: openSidebar, close: closeSidebar, toggle: toggleSidebar } = useSidebar(true);
  ...
  return (
    <div className="flex flex-col h-screen bg-[#0f0f11] text-white overflow-hidden">
      ...
      <div className="flex flex-1 overflow-hidden relative">
        <aside
          className={`bg-[#121212] flex flex-col transition-[width] duration-300 ease-in-out overflow-hidden shrink-0 border-r border-[#1f1f22] ${
            isSidebarOpen ? 'w-56' : 'w-0'
          }`}
        >
          <div className={`flex-1 p-6 flex flex-col gap-6 mt-2 whitespace-nowrap transition-opacity duration-200 ${
          ...
          <div className={`p-6 whitespace-nowrap transition-opacity duration-200 ${
            isSidebarOpen ? 'opacity-100 delay-100' : 'opacity-0 delay-0'
          }`}>
            {accessToken && (
