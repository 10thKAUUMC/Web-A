...
import { ACCESS_TOKEN_KEY, REFRESH_TOKEN_KEY, USER_NAME_KEY } from '../constants/key';
...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  const [userName, setUserNameToStorage, removeUserNameFromStorage] =
    useLocalStorage<string | null>(USER_NAME_KEY, null);
  ...
  const login = async (signInData: RequestSignInDTO) => {
    try {
      ...
      if (result.status) {
        ...
        if (signInData.email) {
          setUserNameToStorage(signInData.email.split('@')[0]);
        }
      }
    } catch (error) {
